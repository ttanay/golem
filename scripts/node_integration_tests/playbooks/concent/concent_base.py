...
class ConcentTestPlaybook(NodeTestPlaybook):
    def step_clear_nodes_output(self, node_id: NodeId):
        helpers.clear_output(self.output_queues[node_id])
        self.next()

    def step_is_node_concent_off(self, node_id: NodeId):
    ...
    def step_enable_node_concent(self, node_id: NodeId):
        def on_success(_):
            self.next()
    ...
    def step_ensure_node_concent_on(self, node_id: NodeId):
        def on_success(result):
            if result is True:
    ...
    initial_steps = NodeTestPlaybook.initial_steps + (
        partial(step_is_node_concent_off, node_id=NodeId.provider),
        partial(step_enable_node_concent, node_id=NodeId.provider),
        partial(step_ensure_node_concent_on, node_id=NodeId.provider),

        partial(step_is_node_concent_off, node_id=NodeId.requestor),
        partial(step_enable_node_concent, node_id=NodeId.requestor),
        partial(step_ensure_node_concent_on, node_id=NodeId.requestor),

        partial(step_clear_nodes_output, node_id=NodeId.requestor),
        partial(step_clear_nodes_output, node_id=NodeId.provider),
    )
